    ...
    await _lock.synchronized(() async {
      if (_useOnlineMode) {
        ...
        if (_speechService != null) {
          await _speechService!.stop();
          _speechService = null;
          print('SpeechService stopped before initializing Vosk');
        }
        ...
        if (_speechService != null) {
          await _speechService!.stop();
          _speechService = null;
          print('SpeechService stopped and cleared for online mode');
        }
        _isInitialized = await _initializeOnlineSpeech();
  ...
  void dispose() {
    ...
    if (_speechService != null) {
      _speechService!.stop().then((_) {
        _speechService = null;
        print('SpeechService stopped and cleared');
      }).catchError((e) {
        print('Error stopping SpeechService: $e');
      });
    }
    if (_recognizer != null) {
      _recognizer = null;
      print('Recognizer cleared');
    }
    if (_model != null) {
      _model = null;
      print('Model cleared');
