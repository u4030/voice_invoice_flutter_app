...
class _VoiceControlWidgetState extends State<VoiceControlWidget> with TickerProviderStateMixin {
  ...
  String _getStatusText(SpeechProvider speechProvider) {
    if (speechProvider.isListening) {
      return 'يستمع الآن... اضغط للإيقاف';
      ...
      return 'في وضع الاستعداد... قل "مرحبا كنان" أو اضغط على الميكروفون';
    } else {
  ...
  void _toggleListening(SpeechProvider speechProvider) {
    // If the app is actively listening for a command, stop it.
    if (speechProvider.isListening) {
      speechProvider.stopListening();
    } else {
      // Otherwise, start listening for a command immediately.
      // This will override the wake word listening if it's active.
      speechProvider.startListening(continuous: true);
    }
  }
